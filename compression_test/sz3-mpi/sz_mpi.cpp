...
/*
template <class T>
void compress(T *data, char *cmpPath, SZ::Config conf)
{
...
int main(int argc, char *argv[])
{
    ...
    if (velocity == 1)
    {
        ...
        for (int ii = 0; ii < compress_it_num; ii++)
        {
            ...
            char *bytes_Vx = SZ_compress<float>(conf, compress_Vx, outSize_Vx);
            char *bytes_Vy = SZ_compress<float>(conf, compress_Vy, outSize_Vy);
            char *bytes_Vz = SZ_compress<float>(conf, compress_Vz, outSize_Vz);
            ...
            if (reconstructed == 1)
            {
                float *reconstructed_Vx = SZ_decompress<float>(conf, bytes_Vx, outSize_Vx);
                float *reconstructed_Vy = SZ_decompress<float>(conf, bytes_Vy, outSize_Vy);
                float *reconstructed_Vz = SZ_decompress<float>(conf, bytes_Vz, outSize_Vz);
