...
void compress_revised(float *in, int dims, int *base, string compressed_file, string io_type, Target target, double target_value, size_t skip_bytes, bool verbose = false, bool debug = false, double *compress_time = NULL)
{
  s.clear();
  ...
  {
    cout << endl
         << "/***** Compression: " << to_string(n) << "D tensor of size " << s[0];
  ...
  if (verbose)
    stop_timer();
  if (debug)
    cout << "Input statistics: min = " << datamin << ", max = " << datamax << ", norm = " << datanorm << endl;
  ...
  compress_time[0] = compress_time[0] + compress_end - compress_start;
