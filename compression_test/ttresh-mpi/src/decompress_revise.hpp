...
void decompress(string compressed_file, float *decompress_data, bool autocrop, bool verbose, bool debug)
{
  vector<Slice> cutout;
  ...
  {
    cout << endl
         << "/***** Decompression: " << to_string(n) << "D tensor of size ";
  ...
  // output_stream.close();
  // printf("buffer_wpos=%d\n", snewprod[n]);
  // decompress_data = (float *)malloc(sizeof(float) * snewprod[n]);
  for (int i = 0; i < snewprod[n]; i++)
  {
    decompress_data[i] = float(c[i]);
  }

  // MPI_Barrier(MPI_COMM_WORLD);
  // double compress_end = MPI_Wtime();
  // decompress_time[0] = decompress_time[0] + compress_end - compress_start;
